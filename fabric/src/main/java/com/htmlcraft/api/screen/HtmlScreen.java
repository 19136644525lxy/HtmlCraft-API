 ...
 * <p>与 Forge 版差异：
 * <ul>
 *   <li>{@code Component} → {@link Text}</li>
 *   <li>{@code GuiGraphics} → {@link DrawContext}</li>
 *   <li>{@code graphics.pose().pushPose()} → {@code graphics.getMatrices().push()}</li>
 *   <li>{@code graphics.pose().translate(...)} → {@code graphics.getMatrices().translate(...)}</li>
 *   <li>{@code graphics.pose().popPose()} → {@code graphics.getMatrices().pop()}</li>
 *   <li>包路径：{@code net.minecraft.client.gui.screens.Screen} → {@code net.minecraft.client.gui.screen.Screen}</li>
 * </ul>
 */
public abstract class HtmlScreen extends Screen {
    ...
    protected HtmlScreen(Text title) {
        super(title);
    }
    ...
    public void render(DrawContext graphics, int mouseX, int mouseY, float delta) {
        // 1. 渲染半透明背景（Yarn: renderBackground(DrawContext)，Forge: renderBackground(GuiGraphics)）
        renderBackground(graphics);
        // 2. 执行 HTML 渲染管线（即时遍历模式：rootLayout 作为参数）
        if (pipeline != null && rootLayout != null) {
            // Yarn: getMatrices() 替代 pose()；push()/pop() 替代 pushPose()/popPose()
            graphics.getMatrices().push();
            graphics.getMatrices().translate(layoutOffsetX, layoutOffsetY, 0);
    ...
    public boolean mouseScrolled(double mouseX, double mouseY, double amount) {
        if (pipeline != null && rootLayout != null) {
            ...
            if (scrollId != null) {
                // amount > 0 = 向上滚，内容相对向上 => scrollY 减小；每步 20px
                pipeline.addScroll(scrollId, (int) (-amount * 20));
                return true;
            }
        }
