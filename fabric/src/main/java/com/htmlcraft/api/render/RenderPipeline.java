...
/**
 * HTML 渲染管线（即时遍历式，Fabric/Yarn 版）。
 * 每个节点按"阴影→背景→边框→文本→子元素"的顺序递归渲染。
 * 支持：scissor 裁剪、渐变背景、圆角裁切、box-shadow、z-index 分层。
 *
 * <p>与 Forge 版差异：
 * <ul>
 *   <li>{@code GuiGraphics} → {@link DrawContext}</li>
 *   <li>{@code graphics.pose()} → {@code graphics.getMatrices()}</li>
 *   <li>{@code pose().pushPose()/popPose()} → {@code matrices().push()/pop()}</li>
 *   <li>{@code enableScissor(x,y,w,h)} → {@code enableScissor(x1,y1,x2,y2)}（Yarn 直接传右下角坐标，非宽高）</li>
 *   <li>{@code drawString(Font, str, float, float, int, boolean)} → {@code drawText(TextRenderer, str, int, int, int, boolean)}</li>
 *   <li>{@code FastColor.ARGB32.*} → {@link ColorHelper.Argb} 的 getAlpha/getRed/getGreen/getBlue</li>
 * </ul>
 */
public class RenderPipeline {
    ...
    /** 构建 + 渲染已合一：直接从布局树渲染到 DrawContext（无需指令缓冲） */
    public void build(LayoutNode layoutNode) {
        // 构建阶段已无必要：立即渲染模式，但保留接口以兼容旧调用
    }

    /** 兼容旧 API：从布局树根节点直接渲染 */
    public void render(DrawContext graphics, LayoutNode layoutNode) {
        if (layoutNode == null) return;
        // 递归时直接按 zIndex 排序 children，再依次渲染
     ...
     * 渲染单个节点及其子树。
     *
     * @param graphics  MC 绘图上下文（Yarn 中为 DrawContext）
     * @param node      当前布局节点
     ...
     */
    private void renderNode(DrawContext graphics, LayoutNode node,
                            int absX, int absY,
        ...
        // ========== 2. 开启 scissor ==========
        // Yarn: enableScissor(x1, y1, x2, y2) 直接传右下角坐标（Forge 同名方法传 x,y,w,h）
     ...
     */
    private void renderTextNode(DrawContext graphics, LayoutNode textNode,
        ...
        if (fontScale != 1.0f && fontScale > 0) {
            // Yarn: drawText 的 x, y 为 int 类型。先 scale 矩阵，再用缩放后的坐标绘制。
            // 原理：scale(s) 后，在坐标 (x/s, y/s) 绘制，视觉上等同于在 (x, y) 绘制 s 倍大小的文本。
            float sx = (float) absX / fontScale;
            float sy = (float) absY / fontScale;
            graphics.getMatrices().push();
            graphics.getMatrices().scale(fontScale, fontScale, 1.0f);
            // drawText 第三、四参数为 int，这里做四舍五入避免精度丢失
            graphics.drawText(context.getFont(), text, Math.round(sx), Math.round(sy), color, false);
            graphics.getMatrices().pop();
        } else {
            graphics.drawText(context.getFont(), text, absX, absY, color, false);
        }
    }
    ...
    // ===== 渐变背景 =====
    private static void drawGradientBg(DrawContext g, int x, int y, int w, int h,
    ...
    private static void fillGradientH(DrawContext g, int x1, int y1, int x2, int y2,
                                      int z, int colL, int colR) {
        if (x2 <= x1) return;
        int w = x2 - x1;
        // 每列 1px 渐变（小场景性能可接受；大场景可做 step 优化）
        // Yarn: 使用 ColorHelper.Argb 替代 Forge 的 FastColor.ARGB32
        float a1 = ColorHelper.Argb.getAlpha(colL) / 255f;
        float r1 = ColorHelper.Argb.getRed(colL)   / 255f;
        float g1 = ColorHelper.Argb.getGreen(colL) / 255f;
        float b1 = ColorHelper.Argb.getBlue(colL)  / 255f;
        float a2 = ColorHelper.Argb.getAlpha(colR) / 255f;
        float r2 = ColorHelper.Argb.getRed(colR)   / 255f;
        float g2 = ColorHelper.Argb.getGreen(colR) / 255f;
        float b2 = ColorHelper.Argb.getBlue(colR)  / 255f;
    ...
    private static void drawBoxShadow(DrawContext g, int x, int y, int w, int h,
    ...
    private static void drawRoundedGradient(DrawContext g, int x, int y, int w, int h, int r,
    ...
    /** 绘制右侧滚动条 */
    private static void drawScrollBar(DrawContext g, int x, int y, int w, int h,
